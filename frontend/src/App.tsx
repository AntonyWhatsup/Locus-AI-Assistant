...
export default function App() {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      ws.current.onopen = () => {
        setMessages([{ from: 'locus', text: 'Connected to Locus API.' }])
      }
      ...
      ws.current.onmessage = (event) => {
        ...
        if (data.type === 'transcript') {
          setMessages(prev => [
            ...prev,
            ...(data.user_text ? [{ from: 'user', text: data.user_text }] : []),
            ...(data.locus_text ? [{ from: 'locus', text: data.locus_text }] : [])
          ])
        }
      }

      ws.current.onclose = () => {
        setMessages(prev => [...prev, { from: 'locus', text: 'Connection lost. Reconnecting...' }])
